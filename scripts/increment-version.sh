...
# Trigger reload on kiosk by updating reload_trigger in database
TRIGGER_VALUE=$(date +%s)
curl -s -X PUT http://127.0.0.1/api/config/reload-trigger \
  -H "Content-Type: application/json" \
  -d "{\"trigger\":$TRIGGER_VALUE}" > /dev/null 2>&1
if [[ $? -eq 0 ]]; then
  echo "Triggered kiosk reload"
fi
