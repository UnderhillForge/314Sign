...
class SignChain:
        ...
        if self.pending_file.exists():
            try:
                with open(self.pending_file, 'r') as f:
                    ...
                    logging.info(f"Loaded {len(self.pending_transactions)} pending transactions")
            except Exception as e:
                logging.error(f"Failed to load pending transactions: {e}")
    ...
    def save_pending_transactions(self):
        ...
        try:
            ...
            logging.error(f"Failed to save pending transactions: {e}")
...
class HardwareVerifier:
    ...
    def _verify_videocore_gpu(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Test vcgencmd availability (version command is most reliable)
...
class SignTokenMiner:
    ...
    def __init__(self, blockchain: SignChain, tokens_file: str = "/var/lib/314sign/tokens.json"):
        ...
        # Hardware verification - cache results to avoid repeated verification
        self.hardware_verifier = HardwareVerifier()
        self.device_role = self.hardware_verifier.get_device_role()
        self._hardware_verification = None  # Cache for verification results
    ...
    def _get_device_model(self) -> str:
        ...
        # Use cached verification result to avoid repeated verification
        if getattr(self, '_hardware_verification', None) is None:
            self._hardware_verification = self.hardware_verifier.verify_mining_eligibility()

        return self._hardware_verification.get('hardware_model', 'Pi 4')  # Default fallback
    ...
    def start_interactive_mining(self):
        """Start interactive mining in the main thread (Ctrl-C to stop)"""
        # HARDWARE SECURITY: Only allow mining on verified Pi hardware
        hardware_check = self.hardware_verifier.verify_mining_eligibility()
        if not hardware_check['eligible']:
        ...
        if chain_info['pending_transactions'] == 0:
            print("❌ No pending transactions to mine!")
            print("   Run --create-test-tx first to create test transactions")
            return False

        print("✅ Hardware verification passed - starting interactive mining")
        print(f"📱 Device: {hardware_check['hardware_model']}")
        print(f"🔒 Anti-spoofing: {'✓' if hardware_check['anti_spoofing_passed'] else '✗'}")
        print(f"📦 Pending transactions: {chain_info['pending_transactions']}")
        print("⛏️ Interactive mining started - Press Ctrl-C to stop")
        print("=" * 50)
        ...
        try:
            while self.mining_active:
                try:
                    ...
                    if throttle_needed:
                        ...
                        print(f"⏸️  Mining throttled: {throttle_reason}")
                        time.sleep(30)  # Shorter sleep for interactive mode
                        continue

                    # Check daily limits
                    daily_exceeded, daily_reason = self._check_daily_limits()
                    if daily_exceeded:
                        print(f"⏹️  Daily mining limit reached: {daily_reason}")
                        print("   Mining will resume tomorrow or when limits reset")
                        break

                    # Check duty cycle
                    if not self._check_duty_cycle():
                        print("⏸️  Outside mining duty cycle - cooling down")
                        time.sleep(15)
                        continue
                    ...
                    current_time = time.time()
                    if current_time - mining_stats['last_status_time'] >= 10:
                        uptime = (current_time - mining_stats['start_time']) / 3600
                        print(f"⏱️  Mining for {uptime:.1f} hours | Blocks: {mining_stats['blocks_mined']} | Rewards: {mining_stats['total_rewards']:.1f}")
                        mining_stats['last_status_time'] = current_time

                    # Mine pending blockchain transactions (verbose mode for interactive)
                    block = self.blockchain.mine_pending_transactions(verbose=True)
                    if block:
                        ...
                        # Show block found message
                        print(f"🎉 BLOCK FOUND! #{block.index} with {len(block.transactions)} TX | Reward: {reward_amount:.1f} 314ST")
                        print(f"   Hash: {block.hash[:32]}...")
                        print(f"   Daily progress: {self.daily_tokens_mined}/{self.daily_token_limits.get(self._get_device_model(), 5)} tokens")

                        # Issue mining reward tokens
                        self._issue_mining_reward(reward_amount, block)

                        # Longer pause after finding a block
                        time.sleep(5)
                    else:
                        # Show mining activity dots
                        print(".", end="", flush=True)
                        time.sleep(1)  # Faster polling for interactive mode
            ...
            print("\n\n⏹️  Mining stopped by user (Ctrl-C)")
            print(f"📊 Final stats: {mining_stats['blocks_mined']} blocks mined, {mining_stats['total_rewards']:.1f} tokens earned")
...
def main():
    ...
    parser.add_argument('--mine-interactive', action='store_true', help='Start interactive mining (Ctrl-C to stop)')
    parser.add_argument('--create-test-tx', action='store_true', help='Create test transactions for mining demo')
    ...
    if args.status:
    ...
    elif args.create_test_tx:
        # Create test transactions for mining demonstration
        ...
        for i in range(5):
            ...
            print(f"✅ Added test transaction: {tx_hash[:16]}...")
        print(f"📦 Created 5 test transactions. Run --mine-interactive to mine them!")
        print(f"💡 Mining should find blocks in 10-60 seconds with difficulty {blockchain.difficulty}")

    elif args.mine_interactive:
        success = token_miner.start_interactive_mining()
        # Note: start_interactive_mining() handles its own output and doesn't return here
        # because it runs in the main thread until interrupted
