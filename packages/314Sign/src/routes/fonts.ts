...
// GET /api/fonts - Return web-safe fonts + custom local fonts
router.get('/', async (req, res) => {
  try {
    ...
    // Add custom fonts (WOFF2/WOFF/TTF supported by modern browsers)
    customFonts.forEach(font => {
      if (font.format === 'WOFF' || font.format === 'WOFF2' || font.format === 'TTF') {
