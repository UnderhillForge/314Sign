...
router.get('/:name', async (req, res) => {
  try {
    ...
    const menuItem: MenuItem = {
      ...
      lastModified: new Date(menu.updated_at),
      font: menu.font,
      fontScalePercent: menu.font_scale_percent
    };
      ...
      if (broadcastToAllRemotes) {
        ...
        for (const rule of rules) {
          ...
          if (days.includes(currentDay)) {
            const startNum = parseInt(rule.start_time.replace(':', ''));
            const endNum = parseInt(rule.end_time.replace(':', ''));
        ...
        // Build update message with current active menu
        const updateMessage = {
          type: 'content-update',
          data: {
            activeMenu: {
              name: activeMenu,
              content: menu.name === activeMenu ? menu.content : ''
            },
            rules,
        ...
        console.log('Broadcasted menu update to synced remotes');
      }
    } catch (error) {
