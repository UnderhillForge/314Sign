<?php
// slideshow-media/upload.php - Handle media uploads for slideshow editor
...
function logUpload($message) {
  ...
  $logFile = $logDir . '/slideshow-media-uploads.log';
...
try {
  ...
  // Validate MIME type (images, SVG, and videos)
  $allowedMimes = [
  ...
  if (!in_array($mimeType, $allowedMimes)) {
    http_response_code(400);
    echo json_encode(['error' => 'Invalid file type. Only images (including SVG) and videos allowed.']);
  ...
  // Additional content validation for non-SVG images
  ...
  $filename = 'media_' . $timestamp . '_' . $uniq . '.' . $ext;
  $mediaDir = __DIR__;
  ...
  if (!move_uploaded_file($file['tmp_name'], $targetPath)) {
    ...
    echo json_encode(['error' => 'Failed to save file', 'detail' => $lastMsg, 'hint' => 'Check slideshow-media directory ownership and writable permissions (run permissions.sh)']);
