...
// Get upload type
$type = $_POST['type'] ?? 'bg';
$uploadDir = dirname(__DIR__) . '/' . ($type === 'logo' ? 'media' : 'bg');
...
if (!is_dir($uploadDir)) {
    if (!mkdir($uploadDir, 0755, true)) {
        ...
        log_entry('error', 'Failed to create upload directory', ['uploadDir' => $uploadDir, 'type' => $type]);
...
if (!is_writable($uploadDir)) {
    ...
    if (!is_writable($uploadDir)) {
        $perm = @fileperms($uploadDir);
        $permStr = $perm ? substr(sprintf('%o', $perm), -4) : 'unknown';
        $owner = @fileowner($uploadDir);
        http_response_code(500);
        log_entry('error', 'upload directory is not writable', ['uploadDir' => $uploadDir, 'type' => $type, 'perms' => $permStr, 'owner_uid' => $owner]);
        echo json_encode(['error' => 'upload directory is not writable', 'hint' => 'Check owner/permissions; run permissions.sh on host to restore www-data ownership']);
        exit;
    }
}
...
if (!move_uploaded_file($file['tmp_name'], $path)) {
    ...
    echo json_encode(['error' => 'Failed to move uploaded file', 'detail' => $lastMsg, 'hint' => 'Check upload directory ownership and disk space; run permissions.sh on host if needed']);
    exit;
}
