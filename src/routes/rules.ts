...
// Helper function to broadcast rule updates to synced remotes
async function broadcastRuleUpdate(req: express.Request) {
  try {
    const broadcastToAllRemotes = req.app.locals.broadcastToAllRemotes;
    if (!broadcastToAllRemotes) return;
    ...
    console.log('Broadcasted rule update to synced remotes');
  } catch (error) {
