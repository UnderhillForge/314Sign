...
if (!isset($_FILES['logo'])) {
    ...
    log_entry('error', 'No file field named "logo" found');
    echo json_encode(['error' => 'No file field named "logo" found.']);
    exit;
}
...
// Basic extension check (support more formats for logos including svg and avif)
...
if (!is_dir($mediaDir)) {
    if (!mkdir($mediaDir, 0755, true)) {
        ...
        log_entry('error', 'Failed to create media directory', ['mediaDir' => $mediaDir]);
...
if (!is_writable($mediaDir)) {
    ...
    @chmod($mediaDir, 0755);
    if (!is_writable($mediaDir)) {
        $perm = @fileperms($mediaDir);
        $permStr = $perm ? substr(sprintf('%o', $perm), -4) : 'unknown';
        $owner = @fileowner($mediaDir);
        http_response_code(500);
        log_entry('error', 'media directory is not writable', ['mediaDir' => $mediaDir, 'perms' => $permStr, 'owner_uid' => $owner]);
        echo json_encode(['error' => 'media directory is not writable', 'hint' => 'Check owner/permissions; run permissions.sh on host to restore www-data ownership']);
        exit;
    }
}

// Save to media/ with unique name
try {
...
if (!move_uploaded_file($file['tmp_name'], $path)) {
    ...
    echo json_encode(['error' => 'Failed to move uploaded file', 'detail' => $lastMsg, 'hint' => 'Check media directory ownership and disk space; run permissions.sh on host if needed']);
    exit;
}
...
log_entry('info', 'Logo upload successful', ['filename' => $filename, 'mime' => $mime, 'size' => $file['size'] ?? null, 'client_ip' => $_SERVER['REMOTE_ADDR'] ?? null]);
